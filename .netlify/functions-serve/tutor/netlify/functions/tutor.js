...
var require_lib2 = __commonJS({
  "node_modules/node-fetch/lib/index.js"(exports2, module2) {
    ...
    function convertBody(buffer, headers2) {
      ...
      let res2, str2;
      if (ct2) {
        res2 = /charset=([^;]*)/i.exec(ct2);
      }
      str2 = buffer.slice(0, 1024).toString();
      if (!res2 && str2) {
        res2 = /<meta.+?charset=(['"])(.+?)\1/i.exec(str2);
      }
      if (!res2 && str2) {
        res2 = /<meta[\s]+?http-equiv=(['"])content-type\1[\s]+?content=(['"])(.+?)\2/i.exec(str2);
        if (!res2) {
          res2 = /<meta[\s]+?content=(['"])(.+?)\1[\s]+?http-equiv=(['"])content-type\3/i.exec(str2);
          if (res2) {
            res2.pop();
          }
        }
      ...
      if (!res2 && str2) {
        res2 = /<\?xml.+?encoding=(['"])(.+?)\1/i.exec(str2);
      }
      if (res2) {
        charset = res2.pop();
    ...
    function fetch2(url, opts) {
      ...
      return new fetch2.Promise(function(resolve, reject) {
        ...
        req.on("response", function(res2) {
          clearTimeout(reqTimeout);
          const headers2 = createHeadersLenient(res2.headers);
          if (fetch2.isRedirect(res2.statusCode)) {
            ...
            switch (request.redirect) {
              ...
              case "follow":
                ...
                if (res2.statusCode !== 303 && request.body && getTotalBytes(request) === null) {
          ...
          if (codings == "deflate" || codings == "x-deflate") {
            const raw = res2.pipe(new PassThrough$1());
...
var tutor_exports = {};
__export(tutor_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(tutor_exports);

// netlify/functions/utils/http.js
var FUNC = "/.netlify/functions/";
var origin = "";
var PORT = process?.env.PORT || 3e3;
var isDev = process?.env.NODE_ENV && ["development", "dev"].includes(process.env.NODE_ENV.toLowerCase());
var isProd = process?.env.NODE_ENV ? true : ["production", "prod"].includes(process.env.NODE_ENV.toLowerCase());
var HOST = isDev ? `http://localhost:${PORT}/` : "/";
var API = HOST + "api/";
var ADMIN = HOST + "admin/";
var headers = (nocache) => ({
  "Access-Control-Allow-Credentials": true,
  "Access-Control-Allow-Headers": "Origin, X-Requested-With, Content-Type, Content-Length, Content-MD5, Accept, Accept-Version, Authorization, X-CSRF-Token, Date, X-Api-Version",
...
var res = (body, code, nocache) => ({
  statusCode: code || 200,
  headers: headers(nocache),
  body: JSON.stringify(body)
});
var makeApi = ({ handlers, connectDB, initAI: initAI2, nocache }) => async (event, context) => {
  ...
  return tryc(
    async () => {
      initAI2 && await initAI2();
      const t2 = handlers[method]?.[q2.type];
      if (!t2)
        return res("", 404);
      if (q2.params)
        q2.params = JSON.parse(q2.params);
      const r2 = await t2(q2, body, event);
      return res(r2 || "done", 200, nocache);
    },
    (e2) => res(e2, 500)
  );
};
var tryc = (func, err) => {
  try {
    ...
    console.error(e2);
    return typeof err === "function" ? err(e2) : err;
  }
};
...
function createBoundary() {
  ...
  let res2 = "";
  while (size--) {
    res2 += alphabet[Math.random() * alphabet.length << 0];
  }
  return res2;
}
...
var APIClient = class {
  ...
  buildRequest(options) {
    ...
    if (this.idempotencyHeader && method !== "get") {
      ...
      headers2[this.idempotencyHeader] = options.idempotencyKey;
    }
    const reqHeaders = this.buildHeaders({ options, headers: headers2, contentLength });
    const req = {
      method,
      ...body && { body },
      headers: reqHeaders,
  ...
  buildHeaders({ options, headers: headers2, contentLength }) {
    const reqHeaders = {};
    if (contentLength) {
...
var createResponseHeaders = (headers2) => {
...
var AbstractAssistantStreamRunner = class {
  ...
  async _threadAssistantStream(body, thread2, options) {
    return await this._createThreadAssistantStream(thread2, body, options);
  }
  ...
  async _createThreadAssistantStream(thread2, body, options) {
    const signal = options?.signal;
    if (signal) {
      if (signal.aborted)
        this.controller.abort();
    ...
    const runResult = await thread2.createAndRun({ ...body, stream: false }, { ...options, signal: this.controller.signal });
    this._connected();
...
var AssistantStream = class _AssistantStream extends AbstractAssistantStreamRunner {
  ...
  static createThreadAssistantStream(body, thread2, options) {
    const runner = new _AssistantStream();
    runner._run(() => runner._threadAssistantStream(body, thread2, {
      ...options,
      headers: { ...options?.headers, "X-Stainless-Helper-Method": "stream" }
    }));
    return runner;
  ...
  async _createThreadAssistantStream(thread2, params, options) {
    const signal = options?.signal;
    if (signal) {
      if (signal.aborted)
        this.controller.abort();
    ...
    const body = { ...params, stream: true };
    const stream = await thread2.createAndRun(body, { ...options, signal: this.controller.signal });
    this._connected();
    for await (const event of stream) {
      __classPrivateFieldGet10(this, _AssistantStream_instances, "m", _AssistantStream_addEvent).call(this, event);
    }
...
var CRYPTO_KEY = Buffer.from("vGdros11d7eJsFS5", "utf8");
var CRYPTO_IV = Buffer.from("c3nPUzoa5LHyUBGI", "utf8");
var aes = (data, key, iv, isEncrypt) => {
  if (!data)
    return "";
  const [from, to] = isEncrypt ? ["utf8", "hex"] : ["hex", "utf8"];
  const create = isEncrypt ? "createCipheriv" : "createDecipheriv";
  const cipher = import_crypto.default[create]("aes-128-cbc", key, iv);
  return `${cipher.update(data, from, to)}${cipher.final(to)}`;
};
var aesDecrypt = (data, key, iv) => aes(data, key, iv, false);
var decrypt = (data, key = CRYPTO_KEY) => aesDecrypt(data, key, CRYPTO_IV);
...
var initAI = async () => {
  ...
  if (!assistant) {
    assistant = await openai.beta.assistants.create({
      name: "David",
      instructions: "You are a professional English tutor. You help me learn English and correct my mistakes.",
      model: "gpt-4o"
    });
  }
};
...
var createMsg = (text) => openai.beta.threads.messages.create(thread.id, {
  role: "user",
  content: text
});
var createRun = () => openai.beta.threads.runs.create(thread.id, {
  assistant_id: assistant.id
});
var runComplete = async (runId) => {
  while (true) {
